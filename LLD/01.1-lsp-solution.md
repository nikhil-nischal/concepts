...
```mermaid
sequenceDiagram
    participant Client as Client code (loop over List<Vehicle>)
    participant Moto as Motorcycle
    participant Car
    participant Bike as Bicycle
    Client->>Moto: hasEngine().toString()
    Moto-->>Client: "true"
    Client->>Car: hasEngine().toString()
    Car-->>Client: "true"
    Client->>Bike: hasEngine().toString()
    Bike-->>Client: null
    Client->>Client: null.toString() → NullPointerException
```
